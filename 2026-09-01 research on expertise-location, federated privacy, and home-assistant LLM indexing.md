...
The academic version of this problem has a name and a closed benchmark: the [TREC Enterprise Track](https://trec.nist.gov/pubs/trec16/papers/ENT.OVERVIEW16.pdf) (2005-2008) ranked employees by expertise from email/document corpora; no major follow-up track since. More recent, code-specific work — [CodeCV](https://www.computer.org/csdl/proceedings-article/scam/2022/960900a143/1JSpk9oqpY4) (IEEE SCAM 2022), RepoSkillMiner — mines commit history via topic modeling to infer domain expertise, with **first authorship and recency of modification the strongest measured correlates of real knowledge** (F-score 71-73% with standard classifiers). That recency finding independently matches this vault's own recency-proximity reranking result: recent activity outranks old activity as a relevance signal, in two unrelated domains now.
...
Trust level changes which part of this problem is even real. Indexing one person's *own* multiple devices (phone, home server, PC) needs none of the above — [Syncthing](https://softhints.com/how-to-sync-obsidian-notes-across-multiple-devices-with-syncthing-a-step-by-step-guide/)-style device sync plus a local search daemon per device is already how tools like Obsidian Sync work, and nothing is being shared with anyone else. The real gap there is purely engineering: **there is no standard off-the-shelf daemon that queries phone + server + PC as one unified index** yet — sync-then-index-locally is solved, unified cross-device querying is still bespoke.

Sharing between fully-trusting, fully-consenting people (a couple, close family) doesn't need SMPC or differential privacy either — it needs access control on a shared space, a solved problem. [Apple Family Sharing](https://adapty.io/blog/app-purchases-family-sharing/) shares by default (opt-out per item); Google's version is opt-in per item; neither does cryptographic zero-knowledge sharing, because neither needs to — full trust makes the hard cross-person privacy problem above disappear. But "full trust" is doing a lot of work in that sentence: a [195-parent survey](https://pmc.ncbi.nlm.nih.gov/articles/PMC11245672/) found only a third of respondents comfortable with fully open family data sharing, most preferring tiered, per-item, evolving consent instead of an all-or-nothing merge — trust between people who love each other is still not the same as trust in a single shared identity.

The sharpest documented warning applies exactly to the couple/family case: **apps built for "share your location/activity with someone you trust" are functionally identical to stalkerware**, and this isn't hypothetical — [research names Life360 explicitly](https://arxiv.org/html/2504.16087v1) alongside dedicated parental-monitoring apps as enabling intimate partner violence through remote access the tracked person didn't consent to or isn't aware of, and Life360 separately faced litigation over selling location data (including children's) to data brokers. A tool built with good intent for a trusted partner is the same tool an abuser wants, and the difference between the two is entirely in the consent model, not the technology — default-on or whole-account sharing is the dangerous shape; explicit, per-item, revocable, visible-to-both-sides opt-in is the safe one.
