...
| Public notes link as `[[public/<note>]]` | `brain` is the `public/` submodule of a private parent vault. That parent vault is not on this machine, so every `public/`-prefixed link (90 notes) resolves to nothing when `brain` is opened standalone | Fine inside the parent vault, broken for standalone/published browsing. Decide which context is authoritative before mass-editing links |
