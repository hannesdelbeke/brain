...
A GitHub search (two rounds) turned up compression tools for LLM context but nothing combining classify-then-cut with a verification gate the way this method does. [marv1nnnnn/llm-min.txt](https://github.com/marv1nnnnn/llm-min.txt) (verified 2026-09-01) distills docs into a dense format for LLM consumption but has no verification step, one-way compression only. PackRat (an auto-learning codebook compressor, cited in the HuangOwen/Awesome-LLM-Compression list) claims lossless round-trip via tiktoken, but that's syntactic codebook substitution, not semantic essential-vs-disposable judgment. [claude-world/skills-optimizer](https://github.com/claude-world/skills-optimizer) (verified 2026-09-01) is the closest conceptual match found: it claims a "fresh read-only validation pass" guaranteeing semantic preservation when compressing Claude Code skills — same shape of idea (compress, then independently check), applied to a different artifact (agent skill files, not vault notes). [luislozanogmia/contextzip](https://github.com/luislozanogmia/contextzip) (verified 2026-09-01) takes the opposite tack this method's `delint()` pre-pass does: it detects code/math/structured data by symbol-density heuristics and routes it around compression entirely, rather than compressing and then checking nothing broke. A Hacker News thread on llm-min.txt independently raised the same idea — use a separate LLM to verify a compressed doc kept task-relevant fidelity — but no shipped repo implements it as a gate against named, checkable elements (links, dates, numbers, code) the way this method does. This method appears to be genuinely unclaimed territory, not a rediscovery.
