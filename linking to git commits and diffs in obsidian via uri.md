...
Add the Git commit code directly to the text of your link (like `[[Note Name@abc1234]]`). Humans just see a normal link, but background scripts can read the code to know exactly which past version you meant.

### Remote Git Permalinks
Use a standard web link pointing to the exact past version of the file on GitHub.com. It's the easiest way to share a past version with others, but requires an internet connection.
...
> [!example]- Portable Wikilink Convention
...
> [!example]- Local Shell Command URI
...
| Need                     | Solution                                      |
| ------------------------ | --------------------------------------------- |
| Permanent offline access | Materialize snapshot                          |
