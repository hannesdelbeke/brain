...
[llmlingua](https://arxiv.org/abs/2310.05736) (microsoft, emnlp 2023) and [llmlingua-2](https://arxiv.org/abs/2403.12968) (microsoft, acl findings 2024) compress prompts and documents for a downstream model to read, which is the pkm note case, not the live-reasoning case. the two papers split the credit differently than this note previously implied: llmlingua (v1) is the one that reports up to 20x compression with about a 1.5 point performance drop, plus the finding that gpt-4 (not gpt-3.5) could reconstruct the original meaning from a heavily-compressed prompt; llmlingua-2 is the one that reframes compression as token classification — a small encoder model learns, per token, whether to keep or drop it from full bidirectional context, rather than the perplexity-based filtering v1 used — and reports smaller headline ratios (2x-5x baseline, up to ~15x in the coarse-to-fine setting v1 introduced) traded for much lower latency and no dependence on a causal LM's logprobs. (verified 2026-09-01)
...
- [recomp](https://arxiv.org/abs/2310.04408) trains a dedicated compressor for RAG-retrieved passages specifically. Needs training infrastructure and a task-specific compressor, aimed at retrieval context, not at compressing a note at write time. Not applicable without building something this vault has no use for elsewhere. (verified 2026-09-01)
...
- two papers argue compression can backfire: [Weinberger et al., "Token Reduction Is Not Cost Reduction"](https://arxiv.org/abs/2607.12161) and a pre-registered trial on compressed-prompt output quality both find that a shorter input can *increase* total cost if it causes longer output, forces a re-read, or if the input was already being served at a prompt-cache discount that a rewrite invalidates. This is the strongest external argument against unconditional compression, and it's the reason the resulting skill (below) filters *what* gets compressed instead of compressing every note that's read. (verified 2026-09-01)
