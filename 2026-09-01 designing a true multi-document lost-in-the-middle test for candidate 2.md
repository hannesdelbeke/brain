...
**superseded below:** the first framing of this section argued a modern small model may have architecturally outgrown the base effect. an adversarial literature check ([[2026-09-01 adversarial literature check for candidate 2]]) found stronger, more specific counter-evidence and a better-supported explanation, kept here in full rather than deleted.

the architecture literature argues against "haiku just grew out of it": [On the Emergence of Position Bias in Transformers](https://arxiv.org/abs/2502.01951) (MIT, ICML 2025) and [Lost in the Middle at Birth](https://arxiv.org/html/2603.10123) (2026) show the U-shaped bias is a geometric property of causal masking and residual connections, present at random initialization before any training, and still present after training on real QA data — a property of every causal transformer, not something a smaller or newer model would age out of. [NoLiMa](https://arxiv.org/abs/2502.05167) (Adobe, ICML 2025) and Chroma's [Context Rot](https://www.trychroma.com/research/context-rot) report both found frontier 2025 models (GPT-4o, Claude 4, Gemini 2.5, and others) still degrade substantially with context length and position once the retrieval isn't a literal string match.

that last clause is the actual explanation. NoLiMa's core finding is that ordinary needle-in-haystack tests, including lost-in-the-middle-style position sweeps, look solved mainly because the needle and the question share literal words — a model can locate the answer by lexical grep instead of position-robust semantic search, and degradation only appears once that overlap is removed. [Facts as First-Class Objects](https://arxiv.org/html/2603.17781) (2026) demonstrates this directly: Claude Sonnet 4.5 gets 100% accuracy up to 97.5% of its context window on structured facts keyed by a unique (drug, target, assay) tuple, with no lost-in-the-middle dip, because the query's unique key gives the attention mechanism an unambiguous target regardless of position.

this experiment's questions ask "why was serial X493-22 recalled" against a unique, verbatim serial number planted in exactly one of 20 documents — the same shape as the drug-target-assay tuples that erased the effect for Claude Sonnet 4.5, and the opposite of Liu et al.'s own setup, where the gold answer is a paraphrased string embedded in Wikipedia prose that a dense retriever (Contriever) had to locate among genuinely topically-close distractors, a semantic search problem rather than an exact-key lookup. a faithful replication of Liu et al.'s own dataset on Llama-2-7B ([LessWrong, "Lost in the Middle" Replicates](https://www.lesswrong.com/posts/udDLgexmJffpMjdDu/lost-in-the-middle-replicates)) reproduced the U-curve, uneven but real.

**the honest reading:** the flat 83.3% most likely reflects that this test's unique-serial-number retrieval step is a lexical lookup the model can solve by exact match, not evidence that claude/haiku has outgrown position bias architecturally. the near-duplicate reason clusters add difficulty to the answer content but not to the retrieval key itself. a version of this experiment built to actually stress the base effect would need to remove that shortcut — an answer identified only by a paraphrasable, non-verbatim description with no unique string shared between question and target document, closer to NaturalQuestions-Open's own answer format than to a serial-number lookup. untested here, and the more promising next step than the bigger-sweep option named below, if this line of research continues.
...
- [[2026-09-01 adversarial literature check for candidate 2]] — found the unique-serial-number lookup key is the likely reason the U-curve didn't reproduce here, replacing the "haiku may have outgrown it" hypothesis above
