...
A Git submodule is an independent `.git` repository. When a note moves across submodule boundaries (such as from root `pkm` to `public/` or from `work/fortis/` to `public/`), standard file operations record a deletion in the source repo and a fresh file creation in the destination repo.
