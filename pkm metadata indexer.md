...
- `skills/pkm-metadata-indexer/index_pkm_meta.py` — scan, parse, embed, and the one hybrid ranker (`search_index`)
- `skills/pkm-metadata-indexer/searchd.py` — resident daemon on `127.0.0.1:44771`, one process for every vault and consumer
- `skills/pkm-metadata-indexer/search_vault.py` — shell client for the daemon, searching in-process when none is running
