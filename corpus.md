...
- **agent session transcripts** (`~/.claude/projects/`, `~/.gemini/antigravity-cli/`): conversation logs across coding assistants ([[Claude Code]], [[antigravity]], [[OpenAI Codex]]), all of them resumed rather than reparsed and all of them dropping tool output to keep secrets out of the index, see [[cross-agent session indexing architecture]]. what a resume means depends on the store: claude code appends jsonl, so it is a [[2026-08-27 tail reads, resuming an index at the byte it stopped at|byte-offset tail read]], while antigravity keeps one sqlite database per conversation holding binary protobuf, where there is no byte to remember and the cursor is the step index.
