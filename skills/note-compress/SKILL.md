...
- `--audit-sample N` — after the run, judge framing-fidelity AND relinking-fidelity on N passed notes, 3 averaged LLM calls each per score (default 0, off)
...
- **`--audit-sample` is a sampled audit, not a second call per note.** The mechanical gate can't catch framing drift, but paying for a judge call on every note doubles the cost of the whole run; sampling multiplies the call count only for the notes checked. Three judge calls per sampled note per score, averaged rather than single-shot or majority-voted, because Rating Roulette (arXiv:2510.27106) measured LLM judges to be highly inconsistent run-to-run on identical input and found averaging repeat calls is what recovers agreement with human judgment.
- **The audit judges two distinct failures, not one blended score.** Framing fidelity (did a hedge or causal connector *survive*) and relinking (did the compression *invent* a new connection between two facts that were separate in the original) are different questions — a compression can ace one and fail the other. KBRA (arXiv:2606.21732, "Safe to Check, Unsafe to Use: Relinking at the Compression Boundary of LLM Agents") names relinking specifically: two individually true fragments ("X happened in March" and, separately, "Y caused delays") get reassembled into a claim neither original sentence made ("X happened in March because Y caused delays"). Neither the mechanical gate (checks spans survive, not what new claims got built from them) nor the framing judge (checks existing connectors survive, not whether a new one got added) catches this, so `--audit-sample` runs a second judge prompt (`RELINK_PROMPT`) and reports both scores per note.
