...
It is not load-bearing on quality. Against 67 judged queries (§3b), no point on the grid is separable from 1:1: every 95% paired-bootstrap interval on MRR contains zero, and 1:1 has the best nDCG@5 outright. The apparent MRR winner, 4:1 at +0.04, wins 12 queries and loses 5 and its interval is [-0.008, +0.088]. Held out — weight picked on 33 queries, scored on the other 34 — the pick *loses* on both metrics: MRR picks 9:1 and gives back -0.033, nDCG picks 2:1 and gives back -0.025. The reordering is churn, not improvement.

**Read that on nDCG@5, not on MRR.** The power check (§3b) deletes a whole retriever and asks whether each metric notices. nDCG@5 does, twice: -0.107 [-0.164, -0.052] for lexical-only and -0.057 [-0.107, -0.007] for vector-only. MRR misses both, and nominally *prefers* vector-only at +0.033 [-0.029, +0.097]. A metric that cannot see an entire retriever removed was never going to see a weight tweak, so the flat MRR column is a fact about MRR at 67 queries, not about the weight. The shuffled null comes back 0/5, so the harness itself is clean. Binned by query length, the whole 4:1 MRR gain sits in one bucket — +0.064 over 22 six-to-nine-word queries, ~0 everywhere else — which is an impulse on a small bin, the shape §3b exists to expose. For scale on how solid these numbers are: re-running the same sweep after one note was added to the vault moved 4:1 from +0.033 to +0.040.
...
Three more checks run by default whenever labels are given (`--no-preflight` turns them off). None of them costs a judgement, and each answers a question that makes the table above unreadable if left open:

- **Power check** — each metric scored against a whole retriever deleted, a change far larger than anything on the grid. A metric that misses *that* cannot be quoted as evidence that the weight does nothing; its flat column is a statement about the metric. This is the check that disqualified MRR on this corpus. Run it before believing any null result.
- **Shuffled null** — the same paired test with the label sets permuted across queries, so no real effect can survive. A significant round here means an artifact: an off-by-one in the pairing, a metric that rewards label-set size, a leak from ranking into labels. Zero across the rounds is the licence to read the real table.
- **Binned by query length** — the per-query delta split into 1-2 / 3-5 / 6-9 / 10+ word buckets. An average of zero has two opposite explanations, "the weight does nothing" and "it helps one kind of query and hurts another", and only the split tells them apart. It also exposes the other failure: a gain that appears in exactly one small bucket is an impulse, not a trend.

All three are five-minute checks that routinely make a long sweep unnecessary, and all three were skipped on the first fusion-weight pass — the power check in particular would have flagged MRR before any of its numbers were written down. They are on by default so the next sweep cannot quietly skip them too.
