...
- **On a multi-facet query there is no "Semantic Best Match" block, on purpose.** Over the same 12 queries the semantic track contributed **1** note the facet and graph tracks had not already found, across 60 top-5 slots — so it was not adding recall, only reordering, and it reordered away from coverage: best-covered-first fell from 12/12 to 6/12 and mean top-5 coverage from 3.65 to 2.52. It also cost about 85% of the route's latency, and its one body snippet is the largest block in the outline, so gating it saves tokens and time together. The failure is structural, not a tuning problem: one whole-query embedding cannot represent "covers four of five facets". Below two facets it still runs, and should — there is no coverage signal to rank on and cosine similarity is the better judge. Pass `--semantic` (route: `semantic=1`) to force it back on and compare; that is the only way to see the pre-gate behaviour. Note this gate rests on 12 measured queries and a coverage proxy, **not** on a judged relevance eval. [[eval_outline.py]] judges the graph-fusion change and deliberately does **not** judge this gate: its first version stubbed the semantic track to an empty list, which meant both of its arms had no semantic signal at all, and it still went on to recommend relaxing the gate. The stub is gone and the docstring now says graph fusion only. Judging the gate needs the real embedding ranker wired into the eval, and until that happens the gate is the least-evidenced thing in this file — `--semantic` (route: `semantic=1`) is how you check it by hand.
