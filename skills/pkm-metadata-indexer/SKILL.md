...
```
`--outline` (alias `--headers-only`) replaces the usual snippet list with a compact outline under 250 tokens: per note a `N/M` facet-coverage count and the heading line each facet was found on, then a graph-neighbourhood section. `--facets "a,b,c"` overrides the parse — it skips tokenizing *and* the stop list, because a caller naming its own facets has already decided `for` is one. `--hops {1,2}` bounds the graph track. The daemon route takes the same `q`, `limit`, `hops`, `facets`, `vault`, `origin`; a daemon predating the route 404s and the client falls back to a local run with a one-line note on stderr rather than failing, so **restart the daemon to get the fast path**.

Two tracks run in a 2-worker `ThreadPoolExecutor` (SQLite drops the GIL in the C extension) and are fused by Reciprocal Rank Fusion at `RRF_K = 60`, where a track that returns nothing contributes 0 rather than distorting the other. Track 1 is one FTS5 query per facet, intersected by note, each hit anchored to its heading line. Track 2 is a recursive CTE over `edges` weighted Adamic-Adar style at `1.0 / ln(max(2, degree))` with a **soft** hub gate — a node over `HUB_DEGREE = 25` is skipped unless it is itself a facet match, so a map-of-content still conducts when the query is about it. Degree is in + out: a MOC is a hub by out-degree and an in-degree count would score it 1 and wave it through. The cross-encoder is an injected seam (`rerank=`) defaulting to **off**, because reranking the top 10 costs 540ms bare and 2.4s in a process holding a DirectML session, which is every daemon by definition — more than the whole rest of the route. Pass one explicitly if you want it.

**Cost is the structural track's, not the route's.** Measured warm on a 3,900-note vault: `/outline` 175ms against `/search`'s 152ms for the same query, so the facet pass and the graph walk together add about **23ms** on top of whatever the semantic track already costs. The structural track on its own — no embeddings, no model, `structural_search` direct — runs 28-45ms, of which ~23ms is alias expansion's single pass over `notes` and only 4-5ms is the FTS intersection and 3ms the CTE. The design target of "under 50ms warm end to end" is not reachable on a vault this size and never was: the semantic track alone is 152ms of it. Use the 23ms delta as the number that matters, and `structural_search` directly if you want the whole answer inside 50ms.
...
Weights use natural `ln()`, not SQLite's base-10 `log()`: the weight is a product across hops, so changing the base reorders 1-hop against 2-hop rather than scaling uniformly. A graph neighbourhood is **not** merged across corpora — two vaults are two separate link graphs — so `/outline` returns one outline block per vault it was asked about. `tests/test_dual_track.py` covers facet clustering, FTS intersection, the recursive CTE, RRF, and a 30ms structural-only performance gate (measured 3.3ms).
