...
Mathematical Weighting (Model 3 Power-Law + Intent):
    w_size = max(0.005, 1.0 / (N - 1) ** 1.5)
    intent_mult = 0.3 if "auto backup" in msg.lower() else 1.0
    commit_weight = w_size * intent_mult
...
def scan_git_commits(vault_dir: Path):
    """Yield (sha, date, msg, list_of_md_files)."""
...
def update_co_commits(db_path: Path, vault_dir: Path, vault_name: str = "root", rebuild: bool = False) -> tuple[int, int]:
    ...
    try:
        with conn:
            ...
            for sha, date, msg, files in scan_git_commits(vault_dir):
                if not latest_sha:
                    latest_sha = sha
                total_commits += 1
                w = calculate_commit_weight(len(files), msg)
        ...
        return total_commits, edge_count
    finally:
        conn.close()


def query_associations(db_path: Path, note: str, vault: str = "", top: int = 25) -> list[tuple]:
    ...
    try:
        ...
        rows = conn.execute(
            ...
            LIMIT ?
            """,
            (f"%{clean_note}", clean_note, clean_note, f"%/{clean_note}", f"%/{clean_note}") + params + (top,),
        ).fetchall()
        return rows
    finally:
        conn.close()
...
def main():
    ...
    commits, edges = update_co_commits(args.db, vault_dir, args.vault, args.rebuild)
    print(f"Processed {commits:,} multi-file commits -> {edges:,} co-commit edges stored in {args.db}\n")
